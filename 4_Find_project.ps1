...
while ($true) {
    $term = (Read-Host "  Search (blank = main menu)").Trim()
    if ($term -eq "") { Clear-Host; .\Main.ps1; return }
    ...
    foreach ($jf in $jsonFiles) {
        ...
        $match = ($info.ProjectID -and $info.ProjectID -like "*$term*") -or
    ...
    if ($results.Count -eq 0) {
        Write-Host "  No results for '$term'" -ForegroundColor Yellow
    } else {
        Write-Host "  $($results.Count) result(s):" -ForegroundColor Cyan
        Write-Host ""
    ...
    # -- Navigation ------------------------------------------------------------
    $nItems = @()
    if ($results.Count -eq 1) { $nItems += "Open in Explorer" }
    $nItems += "New search"
